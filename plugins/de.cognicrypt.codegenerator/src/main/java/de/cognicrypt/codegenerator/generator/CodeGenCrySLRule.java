...
public class CodeGenCrySLRule extends CryptSLRule {
	...
	public CodeGenCrySLRule(String _className, List<Entry<String, String>> defObjects, List<CryptSLForbiddenMethod> _forbiddenMethods, StateMachineGraph _usagePattern, List<ISLConstraint> _constraints, List<CryptSLPredicate> _predicates) {
		super(_className, defObjects, _forbiddenMethods, _usagePattern, _constraints, _predicates);
	}
	
	public CodeGenCrySLRule(CryptSLRule rule) {
		this(rule.getClassName(), rule.getObjects(), rule.getForbiddenMethods(), rule.getUsagePattern(), rule.getConstraints(), rule.getPredicates());
	}
