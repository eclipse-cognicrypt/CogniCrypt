scope({c0_Algorithm:3, c0_Enum:9, c0_Performance:4, c0_Security:5, c0_SignatureScheme:3, c0_keysize:3, c0_performance:3, c0_security:3});
defaultScope(1);
intRange(-8, 7);
stringLength(27);
...
c0_EC = Clafer("c0_EC").withCard(1, 1);
c0_DSA = Clafer("c0_DSA").withCard(1, 1);
...
c0_RSA.addConstraint(equal(joinRef(join($this(), c0_keysize)), constant(4096)));
c0_EC.extending(c0_SignatureScheme);
c0_EC.addConstraint(equal(joinRef(join($this(), c0_security)), global(c0_Strong)));
c0_EC.addConstraint(equal(joinRef(join($this(), c0_performance)), global(c0_Slow)));
c0_EC.addConstraint(equal(joinRef(join($this(), c0_keysize)), constant(256)));
c0_DSA.extending(c0_SignatureScheme);
c0_DSA.addConstraint(equal(joinRef(join($this(), c0_security)), global(c0_Strong)));
c0_DSA.addConstraint(equal(joinRef(join($this(), c0_performance)), global(c0_Slow)));
c0_DSA.addConstraint(equal(joinRef(join($this(), c0_keysize)), constant(2048)));
