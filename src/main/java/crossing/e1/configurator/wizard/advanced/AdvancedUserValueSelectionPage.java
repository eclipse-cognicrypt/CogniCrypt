...
//			final Group titledPanel2 = createPanel(ClaferModelUtils.removeScopePrefix(taskAlgorithm.getRef().getTargetType().getName()), this.container);
			
			final Group titledPanel2 = createPanel(ClaferModelUtils.removeScopePrefix(taskAlgorithm.getRef().getTargetType().getName()), this.container);
			createConstraints(this.taskClafer, taskAlgorithm, titledPanel2);
		}
	...
	private Group createPanel2(final String name, final Composite parent) {
		...
		final Font boldFont = new Font(titledPanel2.getDisplay(), new FontData("Arial", 12, SWT.NONE));
