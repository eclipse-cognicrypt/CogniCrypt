scope({c0_Algorithm:8, c0_Cipher:2, c0_Digest:3, c0_Fast:8, c0_KeyDerivationAlgorithm:3, c0_Security:4, c0_Slow:8, c0_SymmetricBlockCipher:2, c0_SymmetricCipher:2, c0_Task:3, c0_VeryFast:8, c0_VerySlow:8, c0_description:8, c0_digest:3, c0_iterations:3, c0_keySize:2, c0_name:8, c0_outputSize:3, c0_performance:8, c0_security:8, c1_description:3, c1_outputSize:3});
defaultScope(1);
intRange(-8, 7);
stringLength(48);
...
c0_AES.addConstraint(implies(equal(joinRef(join($this(), c0_keySize)), constant(128)), and(some(join(join($this(), c0_performance), c0_VeryFast)), equal(joinRef(join($this(), c0_security)), global(c0_Modern)))));
c0_AES.addConstraint(implies(greaterThan(joinRef(join($this(), c0_keySize)), constant(128)), and(some(join(join($this(), c0_performance), c0_Fast)), equal(joinRef(join($this(), c0_security)), global(c0_Strong)))));
...
c0_DES.addConstraint(some(join(join($this(), c0_performance), c0_VeryFast)));
